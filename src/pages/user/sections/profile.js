...
// const Profile = () => {
//   // Solo usamos setUser y refreshUserData del contexto
//   const { setUser: updateAuthUser, refreshUserData } = useAuth();
...
//     try {
//       setIsLoading(true);
//       const { data } = await api.get("/custom/v1/get-profile");
...
//     } catch (e) {
//       console.error("Error refreshUserData:", e);
//     }
...
//   // Montaje inicial
//   useEffect(() => {
//     if (isNewRegistration.current) handleForceReload();
//     else fetchUserProfile();
//   }, [fetchUserProfile, handleForceReload]);
...
//       await fetchUserProfile();
//       setSaveMessage('¡Perfil actualizado!'); setSaveStatus('success');
//     } catch (err) {
//       console.error(err);
//       setSaveMessage(err.response?.data?.message || err.message);
//       setSaveStatus('error');
//     } finally {
//       setIsSaving(false);
//     }
...
//   if (isLoading) return <div className="profile-loading"><div className="loading-spinner"></div><p>Cargando perfil...</p></div>;
//   if (hasError) return <div className="profile-error"><i className="bi bi-exclamation-circle"></i><p>Error cargando perfil.</p><button onClick={handleForceReload}>Intentar de nuevo</button></div>;

//   return (
//     <div className="profile-section">
//       <h2>Your Profile</h2>
//       <form className="profile-form" onSubmit={handleSave}>
//         <SmartInput label="Name" name="first_name" value={formData.first_name} onChange={handleInputChange} required />
//         <SmartInput label="Last Name" name="last_name" value={formData.last_name} onChange={handleInputChange} required />
//         <div className="form-row">
//           <label>Email *</label>
//           <input type="email" value={formData.email} readOnly className="readonly-field" />
//         </div>
//         <SmartInput type="tel" label="Phone" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="+34 623 456 789" />
//         <SmartInput label="Company" name="company" value={formData.company} onChange={handleInputChange} placeholder="Nombre de tu empresa" />
//         <SmartInput label="Country" name="country" value={formData.country} onChange={handleInputChange} placeholder="Ej: España" />
//         <SmartInput label="State/Province" name="state" value={formData.state} onChange={handleInputChange} placeholder="Ej: Madrid" />
//         <SmartInput label="City" name="city" value={formData.city} onChange={handleInputChange} placeholder="Ej: Getafe" />
//         <SmartInput label="Job" name="job" value={formData.job} onChange={handleInputChange} placeholder="Ej: Web Developer" />
...
//           <PasswordInput label="Current Password" value={passwordData.current_password} onChange={e => setPasswordData(prev => ({ ...prev, current_password: e.target.value }))} required />
//           <PasswordInput label="New Password" value={passwordData.new_password} onChange={e => setPasswordData(prev => ({ ...prev, new_password: e.target.value }))} required minLength={8} />
//           <PasswordInput label="Confirm New Password" value={passwordData.confirm_password} onChange={e => setPasswordData(prev => ({ ...prev, confirm_password: e.target.value }))} required />
//           <div className="form-actions">
//             <button type="submit" disabled={isChangingPassword || !passwordData.current_password || !passwordData.new_password || !passwordData.confirm_password} className="save-button">

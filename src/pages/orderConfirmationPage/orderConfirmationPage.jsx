  ...
  async getOrder(orderId, token, { sessionToken } = {}) {
    const res = await fetch(`${API_URL}/olawee/v1/orders/${orderId}`, {
      headers: {
        ...
        // Si tu backend lo usa, descomenta:
        // 'X-Origin': 'app',
    ...
    // Tu endpoint puede devolver { order: {...} } o directamente el objeto:
      ...
      <div className="confirmation-container">
        ...
        <div className="order-details-card">
          ...
          <div className="order-summary">
            <div className="summary-row">
              <span>Subtotal:</span>
              <span>{isFreeTrial ? '0€' : `${safeTotal.toFixed(2)}€`}</span>
            </div>
            {!isFreeTrial && safeTotal > 0 && (
              <div className="summary-row">
                <span>IVA (aprox.):</span>
                <span>{(safeTotal * 0.21).toFixed(2)}€</span>
              </div>
            )}
            <div className="summary-total">
              <span>Total pagado hoy:</span>

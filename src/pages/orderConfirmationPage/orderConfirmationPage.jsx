...
const api = {
  async getOrder(orderId, token, { sessionToken } = {}) {
    const res = await fetch(`${API_URL}/olawee/v1/orders/${orderId}`, {
      headers: {
...
const OrderConfirmationPage = () => {
  ...
  // Si llega ?trial=true desde el flujo de pago, forzamos vista de prueba gratis
  const isFreeTrialQuery = searchParams.get('trial') === 'true';
  ...
  const fetchOrder = async () => {
    try {
      ...
      const orderData = await api.getOrder(orderId, token, { sessionToken });
      setOrder(orderData);
    } catch (err) {
  ...
  // Consideramos prueba gratis si viene query ?trial=true o si el total del pedido es 0
  const isFreeTrial = isFreeTrialQuery || safeTotal === 0;
        ...
        <div className="order-details-card">
            ...
            {lineItems.map((item, index) => (
              <div key={index} className="order-item">
                ...
                <div className="item-price">
                  {/* 👇 En prueba gratis forzamos 0 €, si no, mostramos el importe del item tal cual devuelve la API */}
                  {isFreeTrial ? '0€' : formatEUR(item.total)}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="order-summary">
            <div className="summary-row">
              <span>Subtotal:</span>
              {/* 👇 Forzamos 0 € si es trial; si no, mostramos el total del pedido */}
              <span>{isFreeTrial ? '0€' : formatEUR(safeTotal)}</span>
            </div>

            {/* ❌ Quitamos la fila de IVA desglosado. El total ya viene con IVA incluido en WooCommerce. */}
            {/* Si quieres, puedes mostrar una nota “IVA incl.” en el total: */}

            <div className="summary-total">
              <span>Total pagado hoy:</span>
              <span>
                {isFreeTrial ? '0€' : `${formatEUR(safeTotal)} `}
                {!isFreeTrial && <small style={{opacity:0.8}}>(IVA incl.)</small>}
              </span>
            </div>

...
// ⚙️ Configura aquí qué productos son "trial" (gratis 14 días)
const TRIAL_PRODUCT_IDS = [78]; // IDs Woo del/los producto(s) con prueba
const FUTURE_CHARGE_EUR = 50;   // precio base después de la prueba (sin IVA)

// ⚙️ IVA (España por defecto)
const TAX_RATE = 0.21; // 21% -> 0.21
...
const api = {
  ...
  async createPaymentIntent(orderId, amountInclVat, token) {
    // amountInclVat llega en euros -> céntimos
    const requestBody = {
      order_id: orderId,
      amount: Math.round(Number(amountInclVat || 0) * 100),
      currency: 'eur',
    };
  ...
  async confirmPayment(orderId, paymentIntentId, token, setupIntentId = null) {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data?.message || 'Error al confirmar el pago');
    return data;
  },
  ...
  async verifyIntent(orderId, paymentIntentId, token) {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data?.message || 'No se pudo verificar el pago con Stripe');
    return data;
  },
};
...
// ──────────────────────────────────────────────────────────────
// Formulario de pago Stripe
function StripePaymentForm({ orderId, amountInclVat, onSuccess, onError, isSetupIntent = false }) {
  const stripe = useStripe();
  const elements = useElements();
  const [isProcessing, setIsProcessing] = useState(false);
  ...
  const handlePayment = async () => {
    ...
    try {
      if (isSetupIntent || amountInclVat === 0) {
        // Setup Intent → guardar método (0€ ahora)
        const { error, setupIntent } = await stripe.confirmSetup({
          elements,
          confirmParams: {
        ...
        if (error) {
          ...
          onSuccess({ id: setupIntent.id, type: 'setup_intent', status: 'succeeded' });
        } else {
          onError('No se pudo completar el guardado del método de pago');
        }
      } else {
  ...
  const formatted = useMemo(() => Number(amountInclVat || 0).toFixed(2), [amountInclVat]);

  return (
    <div className="payment-form">
      <PaymentElement />
      <button onClick={handlePayment} disabled={!stripe || isProcessing} className="btn-pay">
        {isProcessing ? 'Procesando...' : amountInclVat === 0 ? 'Confirmar' : `Pagar ${formatted}€`}
      </button>
    </div>
  );
}
...
const CheckoutPage = () => {
  ...
  // ✅ Evitamos return temprano para no romper las reglas de Hooks
  const product = selectedProduct || { id: 0, price: 0, name: '' };
  const hasProduct = Boolean(selectedProduct);

  // Detectar si el producto seleccionado es "trial"
  const isTrialProduct = useMemo(
    () => TRIAL_PRODUCT_IDS.includes(Number(product.id)),
    [product.id]
  );
  ...
  // Cálculo IVA y totales (hoy)
  const subtotalNow = useMemo(() => (isTrialProduct ? 0 : basePrice), [isTrialProduct, basePrice]);
  const ivaNow = useMemo(() => (isTrialProduct ? 0 : +(subtotalNow * TAX_RATE).toFixed(2)), [subtotalNow, isTrialProduct]);
  const totalInclNow = useMemo(() => +(subtotalNow + ivaNow).toFixed(2), [subtotalNow, ivaNow]);

  // Cálculo IVA y total (después de la prueba)
  const futureBase = useMemo(() => Number(futureCharge ?? FUTURE_CHARGE_EUR), [futureCharge]);
  const futureIva = useMemo(() => +(futureBase * TAX_RATE).toFixed(2), [futureBase]);
  const futureTotalIncl = useMemo(() => +(futureBase + futureIva).toFixed(2), [futureBase, futureIva]);
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      const order = await api.createOrder(orderData, token);
      if (!order.id) throw new Error(order.message || 'Error al crear la orden');

      setOrderId(order.id);

      if (paymentMethod === 'stripe') {
        // amount a Stripe: 0 si trial, total con IVA si no
        const paymentData = await api.createPaymentIntent(order.id, totalInclNow, token);
  ...
  const handlePaymentSuccess = async (paymentDetails) => {
    try {
      ...
      // Tipo de flujo
      const isSetup =
        paymentDetails?.type === 'setup_intent' || isFreeTrial || totalInclNow === 0;
      ...
      if (isStripeSI || isStripePI) {
        try {
          await api.confirmPayment(
            orderId,
            isStripePI ? paymentDetails.id : null,
            token,
            isStripeSI ? paymentDetails.id : null
          );
        } catch (confirmError) {
          console.warn('[Checkout] confirm-payment falló, seguimos con verify-intent si es PI:', confirmError);
        }
        ...
        <div className="checkout-container">
          ...
          <div className="checkout-grid">
            <div className="checkout-main">
                    ...
                    <div className="payment-methods">
                      ...
                      <label className="payment-option">
                        <input
                          ...
                          checked={paymentMethod === 'paypal'}
                          onChange={(e) => setPaymentMethod(e.target.value)}
                          disabled={isTrialProduct} // En trial, forzamos Stripe (guardar método)
                        />
                        <span>PayPal</span>
                      </label>
                    </div>
              ...
              {step === 'payment' && paymentMethod === 'stripe' && clientSecret && (
                <div className="checkout-section">
                  <h2>{totalInclNow === 0 ? 'Guardar método de pago' : 'Completar pago'}</h2>
                  ...
                  <Elements stripe={stripePromise} options={{ clientSecret }}>
                    <StripePaymentForm
                      orderId={orderId}
                      amountInclVat={totalInclNow}
                      onSuccess={handlePaymentSuccess}
                      onError={handlePaymentError}
                      isSetupIntent={totalInclNow === 0}
                    />
                  </Elements>
                </div>
              )}
                          ...
                          purchase_units: [
                            {
                              reference_id: orderId?.toString?.() || '',
                              amount: {
                                value: Number(totalInclNow).toFixed(2), // TOTAL con IVA
                                currency_code: 'EUR',
                              },
                            },
                          ],
            ...
            <div className="checkout-sidebar">
              <div className="order-summary">
                ...
                <div className="product-summary">
                  <div className="product-name">{hasProduct ? product.name : 'Producto no seleccionado'}</div>
                  <div className="product-price">
                    {isTrialProduct ? (
                      <>
                        <span className="trial-badge">Prueba gratuita</span>
                        <span className="future-price">
                          {futureTotalIncl.toFixed(2)}€ (IVA {futureIva.toFixed(2)}€) después de 14 días
                        </span>
                      </>
                    ) : (
                      hasProduct ? (
                        <>
                          <span className="line">Subtotal: {subtotalNow.toFixed(2)}€</span>
                          <span className="line">IVA ({Math.round(TAX_RATE * 100)}%): {ivaNow.toFixed(2)}€</span>
                          <strong className="line">Total (IVA incl.): {totalInclNow.toFixed(2)}€</strong>
                        </>
                      ) : (
                        <span>—</span>
                      )
                ...
                {!isTrialProduct && hasProduct && (
                  <>
                    <div className="summary-row">
                      <span>Subtotal</span>
                      <span>{subtotalNow.toFixed(2)}€</span>
                    </div>
                    <div className="summary-row">
                      <span>IVA ({Math.round(TAX_RATE * 100)}%)</span>
                      <span>{ivaNow.toFixed(2)}€</span>
                    </div>
                  </>
                )}

                <div className="summary-total">
                  <span>Total {totalInclNow === 0 ? 'hoy' : ''}:</span>
                  <span>{totalInclNow === 0 ? '0€' : `${totalInclNow.toFixed(2)}€`}</span>
                </div>

                <div className="summary-benefits">
                  <div>{totalInclNow === 0 ? '14 días de prueba gratuita' : 'Entrega inmediata'}</div>
                  <div>Pago 100% seguro</div>
                  <div>Soporte incluido</div>
                  {totalInclNow === 0 && <div>Cancela cuando quieras</div>}
                </div>
              </div>
            </div>
          </div>

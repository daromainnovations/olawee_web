...
// ✅ NUEVO: Obtener window_id de sessionStorage
const getWindowId = () => {
  return sessionStorage.getItem('window_session_id') || '';
};
...
// ============================================
// FUNCIONES DE AUTENTICACIÓN (ACTUALIZADAS CON WINDOW_ID)
// ============================================

/**
 * LOGIN - Inicia sesión con email y contraseña
 * ✅ Ahora envía window_id
 */
export const loginUser = async (email, password, windowId) => {
  try {
    // Si no se proporciona windowId, obtenerlo de sessionStorage
    const effectiveWindowId = windowId || getWindowId();
    
    console.log('🔑 [LOGIN] Enviando:', { email, windowId: effectiveWindowId });
    ...
    const response = await axios.post(
      ...
      { 
        email, 
        password,
        window_id: effectiveWindowId // ✅ NUEVO
      },
      { 
        timeout: 30000, 
        headers: getCommonHeaders() 
    ...
    if (conflict_detected) {
      return {
        success: false,
        conflict: true,
        message: message || 'Ya existe una sesión activa en otra ventana',
        origin: origin,
        user: userData,
      };
    }
    ...
    if (error.response?.data) {
      ...
      // ✅ Manejar error específico de window_conflict
      if (code === 'window_conflict' || error.response.status === 409) {
        return {
          success: false,
          conflict: true,
          message: message || 'Ya existe una sesión activa en otra ventana',
      ...
      const errorMessages = {
        ...
        'missing_fields': 'Email y contraseña son obligatorios',
        'invalid_email': 'El formato del email es inválido',
        'missing_window_id': 'Error de configuración: window_id no disponible',
      };
 ...
 * ✅ Ahora envía window_id
 */
export const forceLoginUser = async (email, password, windowId) => {
  try {
    // Si no se proporciona windowId, obtenerlo de sessionStorage
    const effectiveWindowId = windowId || getWindowId();
    
    console.log('🔥 [FORCE LOGIN] Enviando:', { email, windowId: effectiveWindowId });
...
/**
 * VALIDATE TOKEN - Valida el token actual
 * ✅ Ahora envía window_id como query param
 */
export const validateToken = async (token, sessionToken) => {
  try {
    const windowId = getWindowId(); // ✅ NUEVO
    
    console.log('🔍 [VALIDATE] Enviando:', { sessionToken, windowId });
    ...
    if (error.response?.data) {
      ...
      // ✅ NUEVO: Manejar invalidación de ventana
      if (code === 'window_invalidated') {
        const enhancedError = new Error(
          message || 'Tu sesión fue cerrada porque iniciaste sesión desde otra ventana'
        );
        enhancedError.code = 'window_invalidated';
        enhancedError.origin = origin;
        enhancedError.originalResponse = error.response.data;
        throw enhancedError;
      }
...
// ============================================
// FUNCIÓN DE REGISTRO (ACTUALIZADA CON WINDOW_ID)
// ============================================
...
export const registerUser = async (email, username, password, extraFields = {}) => {
  try {
    ...
    console.log('📝 [REGISTER] Enviando:', { email, windowId });
